...
**Status: open, and load-bearing.** This is the single motivating use case for inference in the whole design. It requires `is-a` to be genuine transitive subsumption, which is in tension with the SIIM minutes' statement that subtypes inherit nothing — see [00, Issue A](./00-current-understanding.md). It also carries a closed-world assumption: the sweep is only sound if the hierarchy beneath the class is complete *and* the radiologist actually assessed all of it. See [01 §1](./01-what-the-vocabulary-must-express.md). *Update, 2026-09-10: the tension is narrower than stated. The owner ruled that `SUBTYPE_OF` propagates nothing (no element, anatomic scope, or standard clinical metadata edge is inherited), while the classification itself may still be used to test what applies where. Whether the negation sweep is sound remains open.*
